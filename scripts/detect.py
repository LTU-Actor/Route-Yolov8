...
def detect_object():
    ...
    (objects_detected, objects_biggest_bounding_boxes, person_box, sign_box, results_image) = (
        analyze_results(
            infer_image_using(_path=model_u_model_path, _source=latest_img),
            classes={"stop-sign", "tire", "pothole", "person"},
            image_size_in_sq_pixels=(latest_img.shape[0] * latest_img.shape[1]),
        )
    )
    ...
    if config_.enable_vest_mask and person_box is not None:  # Show the vest mask
        ...
        hsv_image = cv2.cvtColor(latest_img, cv2.COLOR_BGR2HSV)
